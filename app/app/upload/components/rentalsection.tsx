    ...
    return (
        <div className="px-[40px]">
            <div className="flex justify-between w-full">
                <div className="flex gap-1 items-center text-[#bab9b9]"><IconAsterisk className="text-theme-3" size={10}/><span>Rent btfs node to upload your data on btfs.</span></div>
                <RentalModalButton/>
            </div>
            <div className="mt-2">
                <Table>
                    ...
                    <TableBody>
                        {files && files.length>0? files.map((file: File)=><FileRow file={file} screen={propDril}/>):
                            <TableRow>
...
function FileRow({file, screen}: {file: File, screen: any}){
    ...
    function customTruncate(hash: string):ReactNode{
        return <p className="text-[15px] cursor-pointer">{hash.slice(0,2)}<span className="text-[12px]">{hash.slice(2,4)}.....{hash.slice(-7)}</span></p>;
    }
    return  <TableRow>
    <TableCell className="font-medium">{file.name}</TableCell>
    {screen.isLargeScreen && <TableCell onClick={()=>{navigator.clipboard.writeText(file.hash)}}>{customTruncate(file.hash)}</TableCell>}
    ...
    <TableCell className="text-right"><a rel="stylesheet"  target="_blank" href={"https://gateway.btfs.io/btfs/"+file.hash}>link</a></TableCell>
    </TableRow>
}